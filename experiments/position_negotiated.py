...
from scripts.game import CooperationGame
...
def create_games(failure_alignment, num_agents):
	...
	for tries in range(min_tries, max_tries+1):
		...
		game = CooperationGame(env, agents, name, umpire=FixedWordOrderGameUmpire(env, store_mean=True))
		games.append(game)
	return games
...
if __name__ == "__main__":
	...
	def signal_handler(signal, frame):
		...
		file_name = "position/negotiated_{0}_{1}_games_{2}_agents.pkl".format(aligned, computed_games, num_agents)
